#!/usr/bin/env python
u"""
load_nodal_corrections.py (09/2017)
...
def load_nodal_corrections(time,constituents):
	...
	cindex = ['sa','ssa','mm','msf','mf','mt','alpha1','2q1','sigma1','q1',
		...
		'l2','t2','s2','r2','k2','eta2','mns2','m3','mk3','s3','mn4','m4','ms4',
		'mk4','s4','s5','m6','s6','s7','s8','z0']
	...
	s,h,p,omega = calc_astrol_longitudes(time)
	hour = (time % 1)*24.0
	t1 = 15.0*hour
	t2 = 30.0*hour
	...
	f[:,5] = np.sqrt((1+0.203*cosn+0.040*cos2n)**2 + (0.203*sinn+0.040*sin2n)**2) #-- Mt

	f[:,6] = 1.0 #-- alpha1
	f[:,7] = np.sqrt((1.+0.188*cosn)**2+(0.188*sinn)**2) #-- 2Q1
	f[:,8] = f[:,7] #-- sigma1
	f[:,9] = f[:,7] #-- q1
	f[:,10] = f[:,7] #-- rho1
	...
	f[:,12] = 1.0 #-- tau1
	#-- Doodson's
	#-- tmp1  = 2.0*np.cos(p*dtr) + 0.4*np.cos((p-omega)*dtr)
	#-- tmp2  = np.sin(p*dtr)+0.2*np.sin((p-omega)*dtr)
	#-- Ray's
	tmp1  = 1.36*np.cos(p*dtr) + 0.267*np.cos((p-omega)*dtr)
	tmp2  = 0.64*np.sin(p*dtr) + 0.135*np.sin((p-omega)*dtr)
	f[:,13] = np.sqrt(tmp1**2 + tmp2**2) #-- M1
	f[:,14] = np.sqrt((1.+0.221*cosn)**2+(0.221*sinn)**2) #-- chi1
	f[:,15] = 1.0 #-- pi1
	f[:,16] = 1.0 #-- P1
	f[:,17] = 1.0 #-- S1
	f[:,18] = np.sqrt((1.+0.1158*cosn-0.0029*cos2n)**2 + (0.1554*sinn-0.0029*sin2n)**2) #-- K1
	f[:,19] = 1.0 #-- psi1
	f[:,20] = 1.0 #-- phi1
	f[:,21] = 1.0 #-- theta1
	f[:,22] = np.sqrt((1.0+0.169*cosn)**2 + (0.227*sinn)**2) #-- J1
	f[:,23] = np.sqrt((1.0+0.640*cosn+0.134*cos2n)**2 + (0.640*sinn+0.134*sin2n)**2 ) #-- OO1
	f[:,24] = np.sqrt((1.0-0.03731*cosn+0.00052*cos2n)**2 + (0.03731*sinn-0.00052*sin2n)**2) #-- 2N2
	f[:,25] = f[:,24] #-- mu2
	f[:,26] = f[:,24] #-- N2
	f[:,27] = f[:,24] #-- nu2
	f[:,28] = 1.0 #-- M2a
	...
	f[:,34] = 1.0 #-- S2
	f[:,35] = 1.0 #-- R2
	f[:,36] = np.sqrt((1.+0.2852*cosn+0.0324*cos2n)**2 + (0.3108*sinn+0.0324*sin2n)**2) #-- K2
	f[:,37] = np.sqrt((1.+0.436*cosn)**2+(0.436*sinn)**2) #-- eta2
	f[:,38] = f[:,29]**2 #-- MNS2
	f[:,39] = f[:,29] #-- 2SM2
	f[:,40] = 1.0 #-- M3 (wrong)
	f[:,41] = f[:,18]*f[:,29] #-- MK3
	...
	u[:,5] = np.arctan(-(0.203*sinn+0.040*sin2n)/(1.0+0.203*cosn+0.040*cos2n))/dtr #-- Mt
	u[:,6] = 0.0 #-- alpha1
	u[:,7] = np.arctan(0.189*sinn/(1.+0.189*cosn))/dtr #-- 2Q1
	u[:,8] = u[:,7] #-- sigma1
	u[:,9] = u[:,7] #-- q1
	u[:,10] = u[:,7] #-- rho1
	u[:,11] = 10.8*sinn - 1.3*sin2n + 0.2*sin3n #-- O1
	u[:,12] = 0.0 #-- tau1
	u[:,13] = np.arctan2(tmp2,tmp1)/dtr #-- M1
	u[:,14] = np.arctan(-0.221*sinn/(1.0+0.221*cosn))/dtr #-- chi1
	u[:,15] = 0.0 #-- pi1
	u[:,16] = 0.0 #-- P1
	u[:,17] = 0.0 #-- S1
	u[:,18] = np.arctan((-0.1554*sinn+0.0029*sin2n)/(1.+0.1158*cosn-0.0029*cos2n))/dtr #-- K1
	u[:,19] = 0.0 #-- psi1
	u[:,20] = 0.0 #-- phi1
	u[:,21] = 0.0 #-- theta1
	u[:,22] = np.arctan(-0.227*sinn/(1.0+0.169*cosn))/dtr #-- J1
	u[:,23] = np.arctan(-(0.640*sinn+0.134*sin2n)/(1.0+0.640*cosn+0.134*cos2n))/dtr #-- OO1
	u[:,24] = np.arctan((-0.03731*sinn+0.00052*sin2n)/(1.0-0.03731*cosn+0.00052*cos2n))/dtr #-- 2N2
	u[:,25] = u[:,24] #-- mu2
	u[:,26] = u[:,24] #-- N2
	u[:,27] = u[:,24] #-- nu2
	u[:,28] = 0.0 #-- M2a
	...
	u[:,34] = 0.0 #-- S2
	u[:,35] = 0.0 #-- R2
	u[:,36] = np.arctan(-(0.3108*sinn+0.0324*sin2n)/(1.+0.2852*cosn+0.0324*cos2n))/dtr #-- K2
	u[:,37] = np.arctan(-0.436*sinn/(1.+0.436*cosn))/dtr #-- eta2
	u[:,38] = u[:,29]*2 #-- MNS2
	u[:,39] = u[:,29] #-- 2SM2
	u[:,40] = 1.50*u[:,29] #-- M3
	u[:,41] = u[:,29] + u[:,18] #-- MK3
	...
	for i,c in enumerate(constituents):
		#-- map between given constituents and supported in OTIS
